...
class OldPhoneDetailsViewModel(private val dataSource: PhonesDataSource) : ViewModel() {

    private val _state = MutableLiveData<PhoneDetailsViewState>()
    val state: LiveData<PhoneDetailsViewState>
        get() = _state

    fun getOldPhoneById(id: Int) {
        viewModelScope.launch {

            _state.postValue(Processing)

            _state.postValue(
                when (val result = dataSource.getOldPhoneById(id)) {
